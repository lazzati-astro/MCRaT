...
//    status = H5Fclose (file);
    ...
    printf("%d,%d,%d,%d\n",dims_2[0],dims_2[1],dims_2[2],dims_2[3]);

    /*
     * Allocate array of pointers to rows.
     */
    rdata = (double **) malloc (dims_2[0] * sizeof (double *));

    /*
     * Allocate space for floating point data.
     */
    rdata[0] = (double *) malloc (dims_2[0] * dims_2[1]  *dims_2[2]*dims_2[3]* sizeof (double));

    /*
     * Set the rest of the pointers to rows to the correct addresses.
     */
    for (i=1; i<dims_2[0]; i++)
        rdata[i] = rdata[0] + i * dims_2[1]*dims_2[2]*dims_2[3];
    ...
    for (i=0; i<2; i++) {
        printf (" [");
        for (j=0; j<dims_2[1]*dims_2[2]*dims_2[3]; j++)
            //for (k=0;k<dims_2[2];k++)
            //    for (l=0;l<dims_2[3];l++)
            printf (" %f", rdata[i][j]);
        printf ("]\n\n");
    }
    printf (" %f\n", rdata[0][0]);
