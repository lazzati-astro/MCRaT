...
The currently available photon types are 'i' for the injected photons and 'o' for ``outlasting'' thermal synchrotron photons (those that have not been absorbed and survive through the MCRaT simulation). 
...
1 \\ \sin(<\theta_p>) \cos(<\phi>) \\ \sin(<\theta_p>) \sin(<\phi>) \\ \cos(<\theta_p>)
\end{pmatrix}
\]
where $\theta_p$ and $\phi$ are the directional angles of the 4 momentum and the $<\cdots>$ denote weighted averages of the various quantities over the photons that fall within bin $(i,j)$. The stokes parameters for the rebinned photon becomes $\tilde{s}=(1,<q>,<u>,<v>)$. The photon weight becomes $\tilde{w}=\sum w_{i,j}$. The position vector of the rebinned photon becomes
 \[
 \overrightarrow{\tilde{r}}_{i,j}=<r>\begin{pmatrix}
 1 \\ \sin(<\theta>) \cos(<\xi>) \\ \sin(<\theta>) \sin(<\xi>) \\ \cos(<\theta>)
 \end{pmatrix}
 \]
 where $<r>$ is the average radius of the photons that lie in bin $(i,j)$, $<\theta>$ is the average polar angle of the photons with the given bins and $\xi$ is a random number between $0$ and $2\pi$. We place the rebinned photon in a random location in the azimuthal direction which does not affect the results, since this feature is only available for 2D HD simulations where we assume cylindrical symmetry to conduct the radiative transfer simulation in 3D.
