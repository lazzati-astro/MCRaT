...
\end{equation}
where $E_\mathrm{max}$ and $E_\mathrm{min}$ are the minimum and maximum energies of the CS photons and $N_E$ is the number of energy bins acquired by multiplying the \texttt{SYNCHROTRON\_REBIN\_E\_PERC} value times the value of the 17$^\textrm{th}$ line of the \texttt{mc.par} file, $N_{17}$.
