...
void readPluto(char pluto_file[STR_BUFFER], struct hydro_dataframe *hydro_data, double r_inj, int ph_inj_switch, double min_r, double max_r, double min_theta, double max_theta, FILE *fPtr)
{
    ...
    for (i=0; i<=array_size[0]; i++) {
		...
		if ( (*grid_x1+i)*HYDRO_L_SCALE >= ph_rmax && (*grid_x1+last_index)*HYDRO_L_SCALE < ph_rmax && (*grid_x1+last_index_buff)*HYDRO_L_SCALE >= ph_rmax) {
			last_index = i;
		} //loop to find index boundaries for x1 region specified by min_r_init and max_r_init in mcrat.c
        ...
        for (j=0;j<nz;j++)
        {
            for (k=0;k<ny;k++)
            {
                for (l=0;l<nx;l++)
                {
                    ...
                    size_t idx=i*(size_t)grid_size+j*(size_t)nz*(size_t)ny+k*(size_t)nx+l;
                    ...
                    if (strcmp(var_strings[i], "rho") == 0)
                    {
                        ...
                        *(x1_buffer+count)= (*(grid_x1+l+first_index)); //offset x1 grid by first_index
						//fflush(fPtr);
                        *(dx1_buffer+count)=(*(grid_dx1+l+first_index)); //offset dx1 by first_index
                        *(x2_buffer+count)=(*(grid_x2+k));
                        *(dx2_buffer+count)=(*(grid_dx2+k));
						//fprintf(fPtr, "NATHAN: storing r = %e, theta = %e\n", (*(x1_buffer+count))*HYDRO_L_SCALE, *(x2_buffer+count));
                        #if DIMENSIONS == THREE
        ...
        while (r_count==0)
        {
            ...
            for (i=0;i<grid_size;i++)
            {
                if (ph_inj_switch==0)
                {
					...
					//fprintf(fPtr, "NATHAN: (ph_rmin-, ph_rmax+) = (%e, %e)\n", ph_rmin - elem_factor*C_LIGHT/hydro_data->fps, ph_rmax + elem_factor*C_LIGHT/hydro_data->fps);
					//fprintf(fPtr, "NATHAN: (r_grid_outercorner, r_grid_innercorner) = (%e, %e)\n", r_grid_outercorner, r_grid_innercorner);
                    if (((ph_rmin - elem_factor*C_LIGHT/hydro_data->fps) <= r_grid_outercorner) && (r_grid_innercorner  <= (ph_rmax + elem_factor*C_LIGHT/hydro_data->fps) ) && (theta_grid_outercorner >= ph_thetamin) && (theta_grid_innercorner <= ph_thetamax) )
                    {
                        r_count++;
                    }
